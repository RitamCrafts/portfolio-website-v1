      ...
      <div className="flex flex-col-reverse md:flex-row gap-x-14 gap-y-7 justify-between items-center">
        ...
        <div className="text-section max-w-150 flex flex-col items-center lg:items-start">
          ...
          <div className="email-loc flex flex-row gap-x-8 gap-y-3 mt-7 mb-6 flex-wrap justify-center">
            <a href={`mailto:${data.contact.email}`} target="_blank" className="email cursor-pointer flex flex-row items-center gap-1 hover:text-orange-500 active:text-orange-500">
              <svg className="pt-1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-mail"><rect width="20" height="16" x="2" y="4" rx="2"></rect><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path></svg>
              <div>{data.contact.email}</div>
            </a>
            <a href={`https://www.google.com/maps/place/${data.contact.location.replaceAll(" ", "+").replaceAll(",","")}/`} target="_blank" className="location cursor-pointer flex flex-row items-center gap-1 hover:text-orange-500 active:text-orange-500">
