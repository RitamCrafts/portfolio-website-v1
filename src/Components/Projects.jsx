        ...
        <div className="heading flex flex-row justify-left items-center gap-3 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            ...
            {projectsShown.map((projectData)=>{
              return <ProjectCard key={projectData.id || projectData.title} project={projectData}/>
            })}
          </div>
      </div>

      <button onClick={()=>setShowAll((prev)=>!prev)} className="px-6 py-3 cursor-pointer mx-auto rounded-lg font-medium transition-all flex items-center gap-2 bg-slate-800 hover:bg-slate-700 active:bg-slate-700 text-white mb-8">
        {(!showAll)?"View All Projects":"Show Less"}
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`lucide lucide-chevron-down ${showAll ? "-rotate-z-180" : ""} transition-transform`}><path d="m6 9 6 6 6-6"></path></svg>
      </button>
    </div>
  )
}
